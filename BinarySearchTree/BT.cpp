...
public:
    BST()
    { //  Initierer 'hode' - ut fra datatypene:
    ...
    void insert(const Data verdi, const MerData merData)
    {
        ...
        if (node)
        { //  Er MINST en node/rot der allerede:
            while (node)
            {               //  Blar til der skal settes inn NEDERST:
            ...
            if (verdi < mor->data)
                mor->left = node; //  Hekter inn til v.
            else
                mor->right = node; //    eller h. ift. 'mor'.
        }
    ...
    {                        //    node med IDen 'verdi':
        ...
        if (!fjernes)
            return false; //  Noden ble IKKE funnet.
        ...
        {                                        // MÅ lete MER etter SEKVENSIELL etterfølger:
    ...
    MerData search(const Data verdi) const
    {
        ...
        if (node)
            return node->merData; //  Funnet - 'merData' returneres.
        else
            return hode->merData; //  IKKE funnet!
    }
};
